...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Future<void> _fetchOrderDetail() async {
    ...
    try {
      ...
      if (token == null) {
        print('DEBUG(OrderDetailScreen): No authentication token found. Redirecting to login.');
        throw Exception("No authentication token found. Please log in.");
      }
      _apiService.setAuthToken(token);
      print('DEBUG(OrderDetailScreen): Token set: ${token.substring(0, 10)}...');

      final SingleOrderResponse response = await _apiService.getOrderDetail(widget.orderId);
      ...
      if (response.data != null) {
        ...
        print('DEBUG(OrderDetailScreen): Order detail fetched for ID: ${widget.orderId}');
      } else {
        setState(() {
          _errorMessage = response.message;
        });
        print('DEBUG(OrderDetailScreen): Order detail data is null or message: ${response.message}');
      }
    } catch (e) {
      if (!mounted) return;
      setState(() {
      ...
      print('DEBUG(OrderDetailScreen): Error fetching order detail: $_errorMessage');
  ...
  Future<void> _handleLogout() async {
    ...
    setState(() { _isLoading = true; }); // Tampilkan loading saat logout
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildDetailRow('Customer ID:', _orderDetail!.customerId?.toString() ?? 'N/A', Icons.person),
                              _buildDetailRow('Service Type:', _orderDetail!.serviceType?.name ?? 'N/A', Icons.wash),
                              _buildDetailRow('Service Option:', _orderDetail!.layanan, Icons.delivery_dining),
                              _buildDetailRow('Status:', _orderDetail!.status, Icons.info_outline, color: _getStatusColor(_orderDetail!.status)),
                              _buildDetailRow('Created At:', _orderDetail!.createdAt?.toLocal().toString().split('.')[0] ?? 'N/A', Icons.calendar_today),
                              _buildDetailRow('Updated At:', _orderDetail!.updatedAt?.toLocal().toString().split('.')[0] ?? 'N/A', Icons.update),
                              ...
                              if (_orderDetail!.status.toLowerCase() == 'baru' || _orderDetail!.status.toLowerCase() == 'proses')
                                SizedBox(
                                  ...
                                  child: ElevatedButton.icon(
                                    onPressed: () => _showCancelOrderDialog(_orderDetail!.id!),
